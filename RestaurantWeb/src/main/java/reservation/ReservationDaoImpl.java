...
public class ReservationDaoImpl implements ReservationDao_InterFace{
	...
	public List<ReservationDto> selectById(String id) throws Exception {
	    List<ReservationDto> list = jdbcTemplate.query("select * from reservation where id = ? AND date >= CURRENT_DATE() ORDER BY date,time ASC", 
	...
	List<ReservationDto> allList = jdbcTemplate.query("select * from reservation where date >= CURRENT_DATE() order by date,time asc",
				new RowMapper<ReservationDto>() {
				@Override
				public ReservationDto mapRow(ResultSet rs, int rowNum) throws SQLException{
					ReservationDto reservationDto = new ReservationDto();
	...
	public List<ReservationDto> todaylist() throws Exception{
		List<ReservationDto> todaylist = jdbcTemplate.query("SELECT * FROM reservation WHERE date = CURDATE() ORDER BY date,time ASC",
			    new RowMapper<ReservationDto>() {
	        @Override
	        public ReservationDto mapRow(ResultSet rs, int rowNum) throws SQLException {
	            ReservationDto reservationDto = new ReservationDto();
