...
#reservationtodaylist th {
  color: #168;
  background: #f0f6f9;
}
#reservationtodaylist th, #reservationtodaylist td {
  padding: 10px;
  border: 1px solid #ddd;
}
#reservationtodaylist th:first-child, #reservationtodaylist td:first-child {
  border-left: 0;
}
#reservationtodaylist th:last-child, #reservationtodaylist td:last-child {
  border-right: 0;
}
</style>
...
<div class="tp">
  ...
  <tbody>
    <c:forEach var="reservationtodaylist" items="${reservationtodaylist}">
      <tr>
        <td>${reservationtodaylist.name}</td>
        <td><c:set var="formattedPhone" value="${reservationtodaylist.phone.substring(0, 3)}-${reservationtodaylist.phone.substring(3, 7)}-${reservationtodaylist.phone.substring(7, 11)}" />
  			${formattedPhone}</td>
        <td>${reservationtodaylist.date}</td>
        <td>${reservationtodaylist.time}</td>
        <td>${reservationtodaylist.person}</td>
        <td>${reservationtodaylist.notice}</td>
        <td><input type="button" value="예약 변경" onclick="resUpdate(this)">
        	<input type="hidden" id="no" name="no" value="${reservationtodaylist.no}"/></td>
